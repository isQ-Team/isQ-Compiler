...
mlir::LogicalResult EliminateHermitianPairs::matchAndRewrite(
    ...
    if (!gate_1.gate_type().hasHint(GateTrait::Hermitian)) {
        return mlir::failure();
    }
    ...
    if (gate_1.gate_type().hasHint(GateTrait::Symmetric)) {
        std::sort(apply_1_args.begin(), apply_1_args.end(),
                  [](mlir::Value a, mlir::Value b) {
                      return a.getImpl() < b.getImpl();
                  });
        std::sort(apply_2_results.begin(), apply_2_results.end(),
                  [](mlir::Value a, mlir::Value b) {
                      return a.getImpl() < b.getImpl();
                  });
    }
