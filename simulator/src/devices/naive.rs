...
impl NaiveSimulator{
    ...
    fn swap_bits(&mut self, q1: &<NaiveSimulator as QDevice>::Qubit, q2: &<NaiveSimulator as QDevice>::Qubit){
        if q1==q2 {return;}
    ...
    fn trace_out_msb_computational_qubit(&mut self, value: bool){
        ...
        if value {
            let mid = self.state.len()/2;
            let (lo, hi) = self.state.split_at_mut(mid);
            unsafe {core::ptr::copy_nonoverlapping(lo.as_mut_ptr(), hi.as_mut_ptr(), hi.len());}
        }
        unsafe{
    ...
    fn allocate_qubit_at_msb(&mut self)->usize{
        self.state.resize(self.state.len()*2, Complex64::new(0.0, 0.0));
    ...
    fn two_qubit_gate(&mut self, pos1: usize, pos2: usize, mat: [[Complex64; 4]; 4]){
        ...
        for i in 0..self.state.len(){
            if (i&(1<<pos1)) ==0 && (i&(1<<pos2))==0{
                ...
                self.state[i+(1<<pos2)] = mat[1][0]*a + mat[1][1]*b + mat[1][2]*c + mat[1][3]*d;
                self.state[i+(1<<pos1)] = mat[2][0]*a + mat[2][1]*b + mat[2][2]*c + mat[2][3]*d;
                self.state[i+(1<<pos1)+(1<<pos2)] = mat[3][0]*a + mat[3][1]*b + mat[3][2]*c + mat[3][3]*d;
            }
        }
    }
...
impl QDevice for NaiveSimulator{
    ...
    fn free_qubit(&mut self, qubit: Self::Qubit) {
        ...
        if prob_zero>=1e-4 && prob_one>=1e-4 {
            warn!("Qubit {} is not in computational state! prob = ({}, {})", qubit, prob_zero, prob_one);
    ...
    fn qop(&mut self, op_type: crate::qdevice::QuantumOp, qubits: &[&Self::Qubit], parameters: &[f64]) {
        ...
        match op_type {
            ...
            X=>{
                self.single_qubit_gate(*qubits[0], [[0.0.into(), 1.0.into()], [1.0.into(), 0.0.into()]]);
            },
            Y=>{
                self.single_qubit_gate(*qubits[0], [[0.0.into(), Complex64::new(0.0, 1.0)], [Complex64::new(0.0, -1.0), 0.0.into()]]);
            },
            Z=>{
                self.single_qubit_gate(*qubits[0], [[1.0.into(), 0.0.into()], [0.0.into(), (-1.0).into()]]);
            },
            H=>{
                self.single_qubit_gate(*qubits[0], [[invsqrt2.into(), invsqrt2.into()], [invsqrt2.into(), (-invsqrt2).into()]]);
            },
            S=>{
                self.single_qubit_gate(*qubits[0], [[1.0.into(), 0.0.into()], [0.0.into(), Complex64::new(0.0, -1.0)]]);
            },
            T=>{
                self.single_qubit_gate(*qubits[0], [[1.0.into(), 0.0.into()], [0.0.into(), Complex64::new(0.0, 1.0)]]);
            },
            CNOT=>{
                self.two_qubit_gate(*qubits[0], *qubits[1], 
            ...
            U3=>{
                ...
                let i = Complex64::i();
                let mat = 
                    [[(theta/2.0).cos().into(), -((i*lambda).exp())*(theta/2.0).sin()], 
                    [(i*phi).exp()*(theta/2.0).sin(), (i*(phi+lambda)).exp()*(theta/2.0).cos()]];
                self.single_qubit_gate(*qubits[0], mat);
            }
            Rx=>{
                let theta = parameters[0];
                let mat = 
                    [[(theta/2.0).cos().into(), (-Complex64::i()*((theta/2.0).sin())).into()], 
                    [(-Complex64::i()*(theta/2.0).sin()).into(), (theta/2.0).cos().into()]];
                self.single_qubit_gate(*qubits[0], mat);
            },
            Ry=>{
                let theta = parameters[0];
    ...
    fn measure(&mut self, qubit: &Self::Qubit)->bool {
        ...
        let (result, prob) = random_coin(prob_zero, prob_one);
        self.collapse_msb_qubit_into(result, prob);
        trace!("Measuring qubit {} (prob = ({}, {})), yielding {}", qubit, prob_zero, prob_one, result);
        result
    }
}
