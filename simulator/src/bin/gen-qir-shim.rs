...
use QIRType::*;
const QIRPauli: QIRType = Other(
    "%Pauli",
    |ssa, qir| {
        let new_ssa = qir.next_ssa();
        (vec![format!("{} = zext %Pauli {} to i8", new_ssa, ssa)], vec![(Pauli, new_ssa)])
    }
);
const CallableTable: QIRType = Other(
    "[4 x void (%Tuple*, %Tuple*, %Tuple*)*]*",
    |ssa, qir| {
        let mut code = Vec::new();
        let address_ssa = vec![qir.next_ssa(), qir.next_ssa(), qir.next_ssa(), qir.next_ssa()];
        let f_ssa = vec![qir.next_ssa(), qir.next_ssa(), qir.next_ssa(), qir.next_ssa()];
        let new_ssa = vec![qir.next_ssa(), qir.next_ssa(), qir.next_ssa(), qir.next_ssa()];
        ...
        for i in 0..4{
            ...
            code.push(format!("{} = load void (%Tuple*, %Tuple*, %Tuple*)*, void (%Tuple*, %Tuple*, %Tuple*)** {}", f_ssa[i], address_ssa[i]));
            code.push(format!("{} = bitcast void (%Tuple*, %Tuple*, %Tuple*)* {} to i8*", new_ssa[i], f_ssa[i]));
        }

        (code, new_ssa.into_iter().map(|x| (QI8P, x)).collect())
    }
);
const MemManTable: QIRType = Other(
    "[2 x void(%Tuple*, i32)*]*",
    |ssa, qir| {
        let mut code = Vec::new();
...
const RotTuple: QIRType = Other(
    "{ i2, double, %Qubit* }*",
    |ssa, qir| {
        let mut code: Vec<String> = vec![];
        let addr_ssa = vec![qir.next_ssa(), qir.next_ssa(), qir.next_ssa()];
        let value_ssa = vec![qir.next_ssa(), qir.next_ssa()];
        let result_ssa = vec![(Pauli, qir.next_ssa()), (Double, qir.next_ssa()), (Qubit, qir.next_ssa())];
        ...
        code.push(format!("{} = load double, double* {}", result_ssa[1].1, addr_ssa[1]));
        code.push(format!("{} = load %Qubit*, %Qubit** {}", value_ssa[1], addr_ssa[2]));
        code.push(format!("{} = zext %Pauli {} to i8", result_ssa[0].1, value_ssa[0]));
        code.push(format!("{} = bitcast %Qubit* {} to i8*", result_ssa[2].1, value_ssa[1]));
...
impl QIRType{
    fn to_qir_type(&self)->&'static str{
        match self{
            ...
            QIRType::QI8P => "i8*",
            QIRType::QI64P => "i64*",
            QIRType::MeasurementProbabilityArgs => "{ %Array, %Array, %Result, double, %String, double }*",
            QIRType::Other(s, _) => s
        }
    }
...
#[derive(Copy, Clone)]
enum PrimitiveType{
    Pointer, Void, I64, I8, I32, F64, I1
}
impl PrimitiveType{
...
impl QIRInterface{
    fn new(category: &'static str, name: &'static str, return_type: QIRType, args: &'static [QIRType])->Self{
        QIRInterface{
            category,
            name,
            return_type,
    ...
    fn rust_shim_name(&self)->String{
        format!("isq_qir_shim_{}_{}", self.category, self.name)
    }
    fn codegen(&mut self)->(String, String, String){
        ...
        let mut qir_args: Vec<(QIRType, String)> = self.args.iter().map(|t| (t.clone(), self.next_ssa())).collect();
        let mut interim_args : Vec<(QIRType, String)> = vec![];
        qir_code.push(format!("define {} @{} ({}) alwaysinline {{\nentry:",
        self.return_type.to_qir_type(), 
        self.qir_name(), 
        qir_args.iter().map(|(t, s)| format!("{} {}", t.to_qir_type(), s)).join(", ")));
        for qir_arg in qir_args.iter(){
            if let Other(_, ssa_expand) = qir_arg.0{
                ...
                qir_code.push(format!("    {} = bitcast {} {} to {}", new_ssa, qir_arg.0.to_qir_type(), qir_arg.1, interim_type));
                interim_args.push((qir_arg.0, new_ssa))
            }
        }
        let call_stmt = format!("call {} @{}({})", 
        self.return_type.to_primitive_type().to_llvm_type(), 
        self.interim_name(),
        interim_args.iter().map(|x| format!("{} {}", x.0.to_primitive_type().to_llvm_type(), x.1)).join(", "));
        if let QVoid = self.return_type{
            ...
            qir_code.push(format!("    {} = bitcast {} {} to {}", cast_ssa, self.return_type.to_primitive_type().to_llvm_type(), ssa, self.return_type.to_qir_type()));
            qir_code.push(format!("    ret {} {}", self.return_type.to_qir_type(), cast_ssa));
        }


        qir_code.push("}".to_owned());
        qir_code.push(format!("declare {} @{}({})", 
        self.return_type.to_primitive_type().to_llvm_type(), 
        self.interim_name(), 
        interim_args.iter().map(|(t, _)| format!("{}", t.to_primitive_type().to_llvm_type())).collect::<Vec<String>>().join(", ")));
        
        // Interim codegen.
        let rust_interim_args = interim_args.iter().map(|x| (x.0, self.next_var_name())).collect_vec();
        interim_code.push(format!("#[no_mangle]\nextern \"C\" fn {}({})->{} {{", self.interim_name(), rust_interim_args.iter().map(|(t, s)| format!("{}: {}", s, t.to_primitive_type().to_rust_type())).join(", "), self.return_type.to_primitive_type().to_rust_type()));
        // We assume that all arguments can be passed through one transmution.
        interim_code.push("    use core::mem::transmute as t;".to_owned());
        interim_code.push(format!("    unsafe {{ t({}({}))}}", self.rust_shim_name(), rust_interim_args.iter().map(|(t, s)| format!("t({})", s)).join(", ")));
        interim_code.push("}".to_owned());
        
        // rust codegen
        rust_code.push(format!("fn {}({})->{}", self.rust_shim_name(), rust_interim_args.iter().map(|(t, s)| format!("{}: {}", s, t.to_rust_type())).join(", "), self.return_type.to_rust_type()));
...
fn qir_builtin()->Vec<QIRInterface>{
    ...
    interfaces.push(QIRInterface::new("rt", "array_concatenate", Array, &[Array, Array]));
    interfaces.push(QIRInterface::new("rt", "array_copy", Array, &[Array, QI1]));
    interfaces.push(QIRInterface::new("rt", "array_create", Array, &[QI32, QI32, QI64]));
    interfaces.push(QIRInterface::new("rt", "array_create_1d", Array, &[QI32, QI64]));
    interfaces.push(QIRInterface::new("rt", "array_get_dim",QI32, &[Array]));
    interfaces.push(QIRInterface::new("rt", "array_get_element_ptr", QI8P, &[Array, QI64P]));
    interfaces.push(QIRInterface::new("rt", "array_get_size", QI64, &[Array, QI32]));
    interfaces.push(QIRInterface::new("rt", "array_get_size_1d", QI64, &[Array]));
    interfaces.push(QIRInterface::new("rt", "array_project", Array, &[Array, QI32, QI64, QI1]));
    interfaces.push(QIRInterface::new("rt", "array_slice", Array, &[Array, QI32, Range, QI1]));
    interfaces.push(QIRInterface::new("rt", "array_slice_1d", Array, &[Array, Range, QI1]));
    interfaces.push(QIRInterface::new("rt", "array_update_alias_count", QVoid, &[Array, QI32]));
    interfaces.push(QIRInterface::new("rt", "array_update_reference_count", QVoid, &[Array, QI32]));
    interfaces.push(QIRInterface::new("rt", "bigint_add", BigInt, &[BigInt, BigInt]));
    interfaces.push(QIRInterface::new("rt", "bigint_bitand", BigInt, &[BigInt, BigInt]));
    interfaces.push(QIRInterface::new("rt", "bigint_bitor", BigInt, &[BigInt, BigInt]));
    interfaces.push(QIRInterface::new("rt", "bigint_bitxor", BigInt, &[BigInt, BigInt]));
    interfaces.push(QIRInterface::new("rt", "bigint_create_array", BigInt, &[QI32, QI8P]));
    interfaces.push(QIRInterface::new("rt", "bigint_create_i64", BigInt, &[QI64]));
    interfaces.push(QIRInterface::new("rt", "bigint_divide", BigInt, &[BigInt, BigInt]));
    interfaces.push(QIRInterface::new("rt", "bigint_equal", QI1, &[BigInt, BigInt]));
    interfaces.push(QIRInterface::new("rt", "bigint_get_data", QI8P, &[BigInt]));
    interfaces.push(QIRInterface::new("rt", "bigint_get_length", QI32, &[BigInt]));
    interfaces.push(QIRInterface::new("rt", "bigint_greater", QI1, &[BigInt, BigInt]));
    interfaces.push(QIRInterface::new("rt", "bigint_greater_eq", QI1, &[BigInt, BigInt]));
    interfaces.push(QIRInterface::new("rt", "bigint_modulus", BigInt, &[BigInt, BigInt]));
    interfaces.push(QIRInterface::new("rt", "bigint_multiply", BigInt, &[BigInt, BigInt]));
    interfaces.push(QIRInterface::new("rt", "bigint_negate", BigInt, &[BigInt]));
    interfaces.push(QIRInterface::new("rt", "bigint_power", BigInt, &[BigInt, QI32]));
    interfaces.push(QIRInterface::new("rt", "bigint_shiftleft", BigInt, &[BigInt, QI64]));
    interfaces.push(QIRInterface::new("rt", "bigint_shiftright", BigInt, &[BigInt, QI64]));
    interfaces.push(QIRInterface::new("rt", "bigint_subtract", BigInt, &[BigInt, BigInt]));
    interfaces.push(QIRInterface::new("rt", "bigint_to_string", QString, &[BigInt]));
    interfaces.push(QIRInterface::new("rt", "bigint_update_reference_count", QVoid, &[BigInt, QI32]));
    interfaces.push(QIRInterface::new("rt", "bool_to_string", QString, &[QI1]));
    interfaces.push(QIRInterface::new("rt", "callable_copy", Callable, &[Callable, QI1]));
    interfaces.push(QIRInterface::new("rt", "callable_create", Callable, &[CallableTable, MemManTable, Tuple]));
    interfaces.push(QIRInterface::new("rt", "callable_invoke", QVoid, &[Callable, Tuple, Tuple]));
    interfaces.push(QIRInterface::new("rt", "callable_make_adjoint", QVoid, &[Callable]));
    interfaces.push(QIRInterface::new("rt", "callable_make_controlled", QVoid, &[Callable]));
    interfaces.push(QIRInterface::new("rt", "callable_update_alias_count", QVoid, &[Callable, QI32]));
    interfaces.push(QIRInterface::new("rt", "callable_update_reference_count", QVoid, &[Callable, QI32]));
    interfaces.push(QIRInterface::new("rt", "capture_update_alias_count", Callable, &[Callable, QI32]));
    interfaces.push(QIRInterface::new("rt", "capture_update_reference_count", Callable, &[Callable, QI32]));
    interfaces.push(QIRInterface::new("rt", "double_to_string", QString, &[Double]));
    ...
    interfaces.push(QIRInterface::new("rt", "pauli_to_string", QString, &[QIRPauli]));
    interfaces.push(QIRInterface::new("rt", "qubit_allocate", Qubit, &[]));
    interfaces.push(QIRInterface::new("rt", "qubit_allocate_array", Array, &[QI32]));
    interfaces.push(QIRInterface::new("rt", "qubit_release", QVoid, &[Qubit]));
    interfaces.push(QIRInterface::new("rt", "qubit_release_array", QVoid, &[Array]));
    interfaces.push(QIRInterface::new("rt", "qubit_to_string", QString, &[Qubit]));
    interfaces.push(QIRInterface::new("rt", "range_to_string", QString, &[Range]));
    interfaces.push(QIRInterface::new("rt", "result_equal", QI1, &[Result, Result]));
    interfaces.push(QIRInterface::new("rt", "result_get_one", Result, &[]));
    interfaces.push(QIRInterface::new("rt", "result_get_zero", Result, &[]));
    interfaces.push(QIRInterface::new("rt", "result_to_string", QString, &[Result]));
    interfaces.push(QIRInterface::new("rt", "result_update_reference_count", QVoid, &[Result, QI32]));
    interfaces.push(QIRInterface::new("rt", "string_concatenate", QString, &[QString, QString]));
    interfaces.push(QIRInterface::new("rt", "string_create", QString, &[QI8P]));
    interfaces.push(QIRInterface::new("rt", "string_equal", QI1, &[QString, QString]));
    interfaces.push(QIRInterface::new("rt", "string_get_data", QI8P, &[QString]));
    interfaces.push(QIRInterface::new("rt", "string_get_length", QI32, &[QString]));
    interfaces.push(QIRInterface::new("rt", "string_update_reference_count", QVoid, &[QString, QI32]));
    ...
    interfaces.push(QIRInterface::new("rt", "tuple_update_alias_count", QVoid, &[Tuple, QI32]));
    interfaces.push(QIRInterface::new("rt", "tuple_update_reference_count", QVoid, &[Tuple, QI32]));
    return interfaces;
}
...
fn qir_microsoft_extension_core()->Vec<QIRInterface>{
    let mut interfaces: Vec<QIRInterface> = Vec::new();
    interfaces.push(QIRInterface::new("qis", "exp__body", QVoid, &[Array, Double, Array]));
    interfaces.push(QIRInterface::new("qis", "exp__adj", QVoid, &[Array, Double, Array]));
    interfaces.push(QIRInterface::new("qis", "exp__ctl", QVoid, &[Array, Array, Double, Array]));
    interfaces.push(QIRInterface::new("qis", "exp__ctladj", QVoid, &[Array, Array, Double, Array]));
    interfaces.push(QIRInterface::new("qis", "h__body", QVoid, &[Qubit]));
    interfaces.push(QIRInterface::new("qis", "h__ctl", QVoid, &[Array, Qubit]));
    interfaces.push(QIRInterface::new("qis", "r__body", QVoid, &[Pauli, Double, Qubit]));
    interfaces.push(QIRInterface::new("qis", "r__adj", QVoid, &[Pauli, Double, Qubit]));
    interfaces.push(QIRInterface::new("qis", "r__ctl", QVoid, &[Array, RotTuple]));
    interfaces.push(QIRInterface::new("qis", "r__ctladj", QVoid, &[Array, RotTuple]));
    ...
    interfaces.push(QIRInterface::new("qis", "s__ctladj", QVoid, &[Array, Qubit]));
    ...
    interfaces.push(QIRInterface::new("qis", "t__ctladj", QVoid, &[Array, Qubit]));
    ...
    interfaces.push(QIRInterface::new("qis", "dumpmachine__body", QVoid, &[QI8P]));
    interfaces.push(QIRInterface::new("qis", "dumpregister__body", QVoid, &[QI8P, Array]));
    return interfaces;
}
...
fn qir_microsoft_extension_foundation()->Vec<QIRInterface>{
    ...
    interfaces.push(QIRInterface::new("qis", "isnegativeinfinity__body", QI1, &[Double]));
    ...
    interfaces.push(QIRInterface::new("qis", "arctan2__body", Double, &[Double, Double]));
    ...
    interfaces.push(QIRInterface::new("qis", "ieeeremainder__body", Double, &[Double, Double]));
    interfaces.push(QIRInterface::new("qis", "drawrandomint__body", Int, &[Int, Int]));
    interfaces.push(QIRInterface::new("qis", "drawrandomdouble__body", Double, &[Double, Double]));
    interfaces.push(QIRInterface::new("qis", "applyifelseintrinsic__body", QVoid, &[Result, Callable, Callable]));
    interfaces.push(QIRInterface::new("qis", "applyconditionallyinstrinsic__body", QVoid, &[Array, Array, Callable, Callable]));
    interfaces.push(QIRInterface::new("qis", "assertmeasurementprobability__body", QVoid, &[Array, Array, Result, Double, QString, Double]));
    interfaces.push(QIRInterface::new("qis", "assertmeasurementprobability__ctl", QVoid, &[Array, MeasurementProbabilityArgs]));
...
fn main(){
    ...
    if std::env::args().len() < 3{
        println!("Usage: {} <preset> <type>", std::env::args().next().unwrap());
        return;
    }
    ...
    if type_name=="qir"{
        println!("%Range = type {{ i64, i64, i64 }}
%Tuple = type opaque
%Qubit = type opaque
%Result = type opaque
%Array = type opaque
