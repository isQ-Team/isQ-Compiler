...
impl QArray{
    pub fn new(elem_size_in_byte: usize, dimensions: &[usize])->Self{
        ...
        let elem_size_in_usize = (elem_size_in_byte + core::mem::size_of::<usize>() - 1 ) / core::mem::size_of::<usize>();
        ...
        let mut dimension_weights = Some(1).into_iter().chain(dimensions.iter().scan(1, |acc, &x| {
            let tmp = *acc;
            *acc *= x;
            Some(tmp)
        })).collect::<Vec<_>>();
    ...
    pub fn concat_1d(&self, other: &Self)->Self{
        ...
        let arr = Self::new(self.elem_size_in_byte, &[self.total_size() + other.total_size()]);
        unsafe{
            let mut i = 0;
    ...
    pub fn get_element(&self, indices: &[i64])->*mut i8{
        if indices.len() != self.dimensions.len(){
            panic!("Index length ({}) does not match array dimension ({})!", indices.len(), self.dimensions.len());
        }
        let mut offset = 0;
        ...
        unsafe{
            self.data.offset((offset * self.elem_size_in_usize()) as isize) as *mut i8
        }
    }
    pub fn slice(&self, index: usize, i: usize)->Self{
        let mut arr = self.project(index, QIRRange{start: i as i64, end: i as i64, step: 1 });
    ...
    pub fn project(&self, index: usize, range: QIRRange)->Self{
        if index>=self.dimensions.len(){
            panic!("Dimension ({}) out of range ({})!", index, self.dimensions.len());
        }
        ...
        */
        ...
        let new_arr = unsafe {core::slice::from_raw_parts_mut(new_array.data, new_array.total_size())};
