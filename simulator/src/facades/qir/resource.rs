...
impl<R> ResourceManagerExt for R where R: ResourceManager{
    fn add<T: Any + 'static>(&mut self, resource: T)->usize{
        let id = self.next();
    ...
    fn get_by_key<T: Any + 'static>(&self, key: ResourceKey<T>)->Option<&T>{
        self.get_any(key.key).and_then(|any| any.downcast_ref::<T>())
    }
    fn get_by_key_mut<T: Any + 'static>(&mut self, key: ResourceKey<T>)->Option<&mut T>{
        self.get_any_mut(key.key).and_then(|any| any.downcast_mut::<T>())
    }
...
impl ResourceManager for ResourceMap{
    ...
    fn update_ref_count(&mut self, id: usize, delta: isize) {
        let pair = self.resources.get_mut(&id).expect(&format!("Resource with id {} not found", id));
        pair.1+=delta;
        if pair.1<0{
...
impl ResourceMap{
    ...
    pub fn try_copy<T: AliasingTracker + 'static>(&mut self, id: usize, force: bool)->usize{
        let current = self.get::<T>(id).expect(&format!("Resource with id {} not found", id));
