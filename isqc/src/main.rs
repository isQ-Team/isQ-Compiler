...
fn main()->miette::Result<()> {
    ...
    match cli.command{
        ...
        Commands::Compile{input, output, opt_level, emit, target, qcis_config, inc_path, int_par, double_par}=>'command:{
            ...
            let compiled_obj = exec::exec_command("", "llc", &["-filetype=obj", "--relocation-model=pic"], &optimized_llvm).map_err(io_error_when("Calling llc"))?;
            // create obj file.
